...
- [ ] Persist and replay multi-step follow-up question sessions end-to-end in the UI
...
- [x] ~~Return structured diagnosis output (category, likely fault, evidence summary, confidence, unknown flag, severity)~~
...
## 4. Feature 4 — Deterministic Resolver Routing
- [x] ~~Implement deterministic routing logic with human-readable rationale~~
- [x] ~~Implement site capability-aware escalation for missing local resolver coverage~~
- [x] ~~Implement EV Driver route~~
- [x] ~~Implement Remote Ops route~~
- [x] ~~Implement Technician route~~
- [x] ~~Implement an active Local Site Resolver routing path (not just schema support)~~
- [ ] Add richer resolver priority and fallback variations for more issue classes

## 5. Feature 5 — Guided Resolution & Escalation Output
- [x] ~~Implement Tier 1 driver action card~~
- [x] ~~Implement Tier 3 remote ops action pack~~
- [x] ~~Implement Tier 4 technician dispatch packet~~
- [x] ~~Implement curated KB snippet retrieval for artifact generation~~
- [x] ~~Implement a real Tier 2 local SOP card path end-to-end~~
- [ ] Add annotated technician evidence / evidence highlight output
- [x] ~~Improve artifact formatting and presentation polish by resolver tier~~

## 6. Demo / evaluation / submission support
- [x] ~~Implement real in-product demo mode~~
- [x] ~~Add 3 seeded demo scenarios~~
...
- [ ] Whenever any item in this checklist changes state, update `docs/progress_tracker.md` in the same session
