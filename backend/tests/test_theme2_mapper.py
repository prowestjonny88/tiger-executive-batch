...
def test_evdb_normalized_wrong_specs_route_to_after_sales(theme2: Theme2VisualExtraction):
    output, _ = build_competition_output(
...
def test_evdb_unreadable_specs_keep_phase_and_request_proof():
    output, _ = build_competition_output(
        IncidentInput(site_id="site-mall-01", photo_hint="EVDB single phase labels unreadable"),
        _perception(
            Theme2VisualExtraction(
                input_component="evdb",
                observation_result="evdb_single_phase",
