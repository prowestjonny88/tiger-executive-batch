...
def main() -> None:
    ...
    if args.resume and (rerun_statuses or rerun_errors):
        raise SystemExit("--resume cannot be combined with --rerun-status or --rerun-errors")
    ...
    if rerun_statuses or rerun_errors:
        existing_labels = load_existing_labels(args.output)
        rows_to_process, preserved = rows_for_rerun(manifest_rows, existing_labels, rerun_statuses, rerun_errors)
    ...
    generate_jsonl_stream(manifest_rows, target_output, max(args.progress_every, 1), append=False)
