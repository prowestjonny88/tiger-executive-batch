...
export function EvidencePanel({ imageUrl, annotations = [] }: EvidencePanelProps) {
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 flex flex-col items-center justify-center relative overflow-hidden min-h-[300px] shadow-sm">
      <div className="mb-3 flex w-full items-center justify-between gap-3 text-slate-900">
        <div>
          <p className="technical-label text-slate-500">Highlighted evidence</p>
          <p className="text-sm font-semibold text-slate-700">Detected visual boxes</p>
        </div>
        {annotations.length > 0 ? <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-bold text-slate-700">{annotations.length} boxes</span> : null}
      </div>
      <EvidenceDialog imageUrl={imageUrl} annotations={annotations}>
        <button type="button" className="w-full cursor-zoom-in rounded-xl bg-white p-2 focus:outline-none focus:ring-2 focus:ring-green-500">
